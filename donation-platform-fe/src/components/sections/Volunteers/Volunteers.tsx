...
export const Volunteers: React.FC = () => {
	...
	const fetchEvents = useCallback(async (shifts: VolunteerShift[] = []) => {
		...
		try {
			...
			const availableEvents = eventsWithCount.filter(
				event => event.currentVolunteers < event.volunteerGoal && !shifts.some(shift => shift.event.id === event.id)
			);
						...
						<div className={styles.cardContent}>
							...
							<div className={styles.volunteerProgress}>
								...
								<div className={styles.progressText}>
									<span>
										{event.currentVolunteers} / {event.volunteerGoal} volontera
									</span>
								</div>
							</div>
							...
							<button className={styles.applyButton} onClick={() => handleRegister(event)}>
